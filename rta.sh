...
echo myDebug=1 myDisable=0 myNoAliasAction=1 gulp runtests  --test=_cax-${BN} | tee $T1 || true
myDebug=1 myDisable=0 myNoAliasAction=1 gulp runtests  --test=_cax-${BN} | tee $T1 || true
cp -r tests/baselines/local  $L1 || return 4
grep "checkExpression return: \|checkSourceElement: " $T1 > $T1.cet.txt

echo myDebug=1 myDisable=0 myNoAliasAction=0 gulp runtests  --test=_cax-${BN} | tee $T0 || true 
myDebug=1 myDisable=0 myNoAliasAction=0 gulp runtests  --test=_cax-${BN} | tee $T0 || true
