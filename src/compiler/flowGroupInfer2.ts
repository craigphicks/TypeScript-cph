...
namespace ts {
    ...
    export function createMrNarrow(checker: TypeChecker, sourceFile: Readonly<SourceFile>, _mrState: MrState, /*refTypesTypeModule: RefTypesTypeModule, */ compilerOptions: CompilerOptions): MrNarrow {
        ...
        // TODO: ensure that checker.getTypeOfSymbol is being called nowhere where floughGetTsTypeOfSymbol should be called instead
        ...
        function flough({sci, expr:expr, inferStatus, qdotfallout, crit, accessDepth, refAccessArgs }: FloughArgs): FloughReturn {
            ...
            if (expr.parent.kind === SyntaxKind.ReturnStatement){
                if (getMyDebug()){
        ...
        function floughInnerAux(qdotfalloutInner: RefTypesTableReturn[]): FloughInnerReturn {
            ...
            function floughInnerVariableDeclaration(): FloughInnerReturn {
                ...
                // TODO: move this out so other functions can use it before calling checker.getTypeFromTypeNode
                function scanTypeNodeForTypeQueryFloughResolveArgs(typeNode: TypeNode, sci: RefTypesSymtabConstraintItem): RefTypesSymtabConstraintItem {
                    const visitor: Visitor = (node: Node): undefined => {
                        if (isRefTypesSymtabConstraintItemNever(sci)) return undefined;
                        if (node.kind===SyntaxKind.TypeQuery){
                            assertCastType<TypeQueryNode>(node);
                            const qmntr = flough({ sci, expr: node.exprName, crit: { kind: InferCritKind.none }, qdotfallout: undefined, inferStatus });
                            const rttr = applyCritNoneUnion(qmntr,inferStatus.groupNodeToTypeMap);
                            if (extraAsserts) Debug.assert(!rttr.logicalObjectAccessData);
                            sci = rttr.sci;
                        }
                        else {
                ...
                if (isIdentifier(expr.name)){
                    ...
                    if (!symbolFlowInfo){
                        ...
                        if (symbol.valueDeclaration===expr) {
                            // primary
                            if (expr.type) {
                                sci = scanTypeNodeForTypeQueryFloughResolveArgs(expr.type,sci);
            ...
            function floughInnerAsExpression(): FloughInnerReturn {
                ...
                // TODO: process typeNode for type queries who args need to be resolved by flough.
            ...
            function floughByCallExpressionV3(): FloughInnerReturn {
                ...
                leftUnmerged.forEach((rttr,rttridx)=>{
                    ...
                    floughLogicalObjectInnerModule.getTsTypesOfBaseLogicalObjects(
                        floughLogicalObjectModule.getInnerIF(functionLogicalObjects)).forEach((tstype,_tstypeidx)=>{
                            tstypes.push(tstype);
                        });
                    ...
                    if (getMyDebug()){
                        ...
                        tstypes.forEach((tstype,tstypeidx)=>{
                            consoleLog(`tstypes[${tstypeidx}]: ${checker.typeToString(tstype)}`);
                        });
                    }
                    ...
                    tstypes.forEach((tstype,tstypeidx)=>{
                        ...
                        let finished = false;
                        {
                            ...
                            arrsig.forEach((sig,sigidx)=>{
                                ...
                                if (getMyDebug()){
                                    dbgRefTypesSymtabConstrinatItemToStrings(tmpSC).forEach(s=>consoleLog(`floughByCallExpression[start one sig], rttridx:${rttridx} sigidx:${sigidx},tmpSC: ${s}`));
                                }
                                let pass1 = resolvedCallArguments.every((carg,cargidx)=>{
                                    consoleLog(`floughByCallExpression[start one arg], rttridx:${rttridx} sigidx:${sigidx}, cargidx:${cargidx}, carg:${floughTypeModule.dbgFloughTypeToString(carg.type)}`);
                                    /**
                                    ...
                                    if (carg.symbol) {
                                        ({type: assignableType, sc:tmpSC}=andSymbolTypeIntoSymtabConstraint({
                                            symbol:carg.symbol,isconst:carg.isconst,type:assignableType,sc: tmpSC,getDeclaredType: getEffectiveDeclaredTypeFromSymbol, mrNarrow
                                        }));
                                    }
                                ...
                                if (pass1){
                                    ...
                                    if (!floughTypeModule.isNeverType(functionSigRtnType)){
                                        ...
                                        if (getMyDebug()){
                                            consoleLog(`floughByCallExpression[skipping because never function return type] rttridx(in):${rttridx}/${leftUnmerged.length}, tstypeidx:${tstypes.length}, sigidx:${sigidx}/${arrsig.length}`);
                                        }
                                    }
                                }
                            }); // sigidx
                        ...
                        if (getMyDebug()){
                            consoleLog(`floughByCallExpression sigGroupFailedCount:${sigGroupFailedCount}`);
                        }
                        if (getMyDebug()){
                            consoleLog(`floughByCallExpression[dbg rttridx:${rttridx}/${leftUnmerged.length}, tstypeidx: ${tstypeidx}/${tstypes.length}, finished:${finished}`);
                        }
                    }); // tstypes.forEach
                ...
                if (leftExpr.kind===SyntaxKind.Identifier) {
                    ...
                    if (!symbolFlowInfo){
                        ...
                        if ((symbol.valueDeclaration as VariableDeclaration).type){
                            // TODO: flough shouldn't need to worry about typeNodeTsType, that's for type checking.
                            // However, process typeNode for type queries who args need to be resolved by flough
                    ...
                    if (getMyDebug()){
                        consoleLog(`floughByBinaryExpresionAssign[out] ${Debug.formatSyntaxKind(expr.left.kind)}`);
                        consoleGroupEnd();
                    }
                    return { unmerged, };
                }
            ...
            function floughAccessExpressionCritNone(): FloughInnerReturn {
                ...
                if (getMyDebug()){
                    consoleGroup(`floughAccessExpressionCritNone[in] expr: ${dbgsModule.dbgNodeToString(expr)}, accessDepth:${accessDepth}`);
                }
                ...
                if (getMyDebug()){
                    consoleLog(`floughAccessExpressionCritNone[out] expr: ${dbgsModule.dbgNodeToString(expr)}, accessDepth: ${accessDepth}`);
                    consoleGroupEnd();
                }
                    ...
                    const pushOne = (logicalObject: FloughLogicalObjectIF, key: string, tf: FloughType) => {
                        ...
                        if (rttr0.symbol){
                            ({sc} = andSymbolTypeIntoSymtabConstraint({
                                ...
                                sc, getDeclaredType: getEffectiveDeclaredTypeFromSymbol, mrNarrow
                            }));
                        }
                        if (keySymbol) {
                            ({sc} = andSymbolTypeIntoSymtabConstraint({
                                symbol: keySymbol, isconst: keyMntr.unmerged[0].isconst,
