...
echo myDbgOutFilename=$T0 myDisableInfer=0 gulp runtests  -t=${BN}
myDbgOutFilename=$T0 myDisableInfer=0 gulp runtests  -t=${BN} || true
cp -r tests/baselines/local  $L0 || return 3
#grep "checkExpression return: \|checkSourceElement: "  $T0 > $T0.cet.txt

echo myDbgOutFilename=$T1 myDisableInfer=1 gulp runtests  -t=${BN}
myDbgOutFilename=$T1 myDisableInfer=1 gulp runtests  -t=${BN} || true
